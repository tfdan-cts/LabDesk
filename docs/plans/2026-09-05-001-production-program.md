...
| 2. Installer, signing, iOS | WP20 update signing turned on end to end with a real key ceremony; Authenticode signing step live on secret; MSI and installer hardening; the iOS application built in CI, tested in the simulator, TestFlight ceremony documented | A signed build measured on a clean Windows machine (SmartScreen state recorded, not inferred); an ipa built by CI; simulator test log | Windows half not started; certificate is owner-only. iOS lane started 2026-09-05 on `feat/ios`, tracked in `docs/IOS.md`. Done: the reason the iOS CI job reports skipped is established (its own `if: inputs.upload-artifact`, which Full Flutter CI passes as false), a simulator verification workflow exists that boots an iPhone on a macOS runner and uploads screenshots, and the target carries LabDesk's identity (`net.lab-desk.LabDesk`, LabDesk name and icon, no upstream team id, no push entitlement). Not done: a green simulator run, any Tier 1 screen, and the encryption declaration. TestFlight and the App Store stay blocked on the Apple Developer account. |
