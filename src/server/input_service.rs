...
impl VirtualInputState {
    fn new() -> Option<Self> {
        VirtualInput::new(
            ...
            // 1. Press "Command" key in LabDesk
            // 2. Move the physical mouse
            // 3. Press "V" key in LabDesk
            ...
            // we consider only LabDesk operates the controlled side.
            //
...
fn key_sleep() {
    ...
    // Though `/Applications/LabDesk.app/Contents/MacOS/labdesk --server` in terminal is ok.
    let now = Instant::now();
...
pub async fn lock_screen() {
    ...
    if #[cfg(target_os = "linux")] {
        ...
        // loginctl lock-session also not work, they both work run labdesk from cmd
        std::thread::spawn(|| {
