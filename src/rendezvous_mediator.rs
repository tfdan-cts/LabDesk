...
// `labdesk --deploy --token <api_token>` there is no point re-running the
...
impl RendezvousMediator {
        ...
        loop {
            ...
            select! {
                ...
                _ = timer.tick() => {
                    ...
                    // until the operator runs `labdesk --deploy`.
            ...
            Some(rendezvous_message::Union::RegisterPkResponse(rpr)) => {
                ...
                match rpr.result.enum_value() {
                    ...
                    Ok(register_pk_response::Result::NOT_DEPLOYED) => {
                        if !NEEDS_DEPLOY.load(Ordering::SeqCst) {
                            log::warn!("Server requires deployment. Run `labdesk --deploy --token <api_token>` on this device.");
                        }
    ...
    async fn register_pk(&mut self, socket: Sink<'_>) -> ResultType<()> {
        ...
        // `labdesk --deploy --token <api_token>`.
        if NEEDS_DEPLOY.load(Ordering::SeqCst) {
