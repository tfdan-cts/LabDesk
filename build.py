...
hbb_name = 'labdesk' + ('.exe' if windows else '')
...
def get_features(args):
    ...
    if args.drm:
        ...
        # package NAMED `labdesk` carrying the consent-bypass backend and the root-side uinput
...
# The package used to be built under the upstream name `rustdesk`, and it owns the same paths it
# always did, so an existing install has to be taken over rather than collided with: Replaces lets
# dpkg hand the files over, Conflicts makes apt remove the old package in the same transaction, and
# Provides keeps anything that depended on the old name satisfied. Without these,
# `apt install ./labdesk-*.deb` on a machine carrying the old build fails on every shared path.
DEB_OLD_PACKAGE_NAME = 'rustdesk'
...
def generate_control_file(version):
    ...
    content = """Package: labdesk
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: LabDesk contributors <noreply@lab-desk.net>
Homepage: https://lab-desk.net
...
# commit — the same way labdesk sources its other native build deps (vcpkg,
# flutter_rust_bridge, ...), rather than carrying a git submodule. It is the ONLY
# pin for the drm backend: labdesk dlopens this .so at runtime and does not depend on
...
def build_libdrmtap_so():
    ...
    # the shared one explicitly (labdesk dlopens the .so and never needs the archive).
...
# dpkg identity AND the release file name -- they used to differ only because the package still
# carried the upstream name while the file already carried the product's, and there is no reason
# for a user to see two different names for one artifact.
DRM_PACKAGE_NAME = 'labdesk-unattended-wayland'
# The same variant was published under the upstream name before the rename; see DEB_OLD_PACKAGE_NAME.
DRM_OLD_PACKAGE_NAME = 'rustdesk-unattended-wayland'
...
def stage_libdrmtap_into_deb(so_path):
    ...
    # symlink is needed: libdrmtap is resolved by ABSOLUTE path (/usr/lib/labdesk/libdrmtap.so.0) at
    # the in-process dlopen site (drmtap_dl.rs), so the deb does NOT drop /usr/lib/labdesk into the
    ...
    system2('mkdir -p tmpdeb/usr/lib/labdesk')
    ...
    system2(f'cp "{so_path}" tmpdeb/usr/lib/labdesk/')
    system2(f'ln -sf "{so_basename}" tmpdeb/usr/lib/labdesk/libdrmtap.so.0')


def _max_glibc_minor(path):
    # Read from .dynstr rather than via objdump so packaging needs no binutils; chunked because
    # liblabdesk.so is ~45 MB.
    best = 0
...
def measured_glibc_floor():
    ...
    paths = [p for p in glob.glob('tmpdeb/usr/lib/labdesk/libdrmtap.so.0.*')
             + glob.glob('tmpdeb/usr/share/labdesk/lib/liblabdesk.so')
             + glob.glob('tmpdeb/usr/share/labdesk/labdesk')
...
def retarget_control_to_drm_variant():
    ...
    with open(path) as f:
        lines = f.readlines()
    out = []
    # The stock control file already carries Conflicts/Replaces/Provides for the pre-rename package
    # name. dpkg rejects a control file that repeats a field, so drop those and emit one set here
    # naming everything this variant takes over: the stock package under both its names, and the
    # variant's own pre-rename name.
    superseded = ', '.join(('labdesk', DEB_OLD_PACKAGE_NAME, DRM_OLD_PACKAGE_NAME))
    for line in lines:
        if line.startswith('Package: labdesk'):
            out.append(f'Package: {DRM_PACKAGE_NAME}\n')
            out.append(f'Conflicts: {superseded}\nReplaces: {superseded}\nProvides: {superseded}\n')
            ...
            out.append(line)
    body = ''.join(out)
    # Fail loudly rather than silently shipping a package that says `labdesk`: a stock control file
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/labdesk')
    system2('mkdir -p tmpdeb/etc/labdesk/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/labdesk/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/labdesk || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/labdesk/')
    system2(
        'cp ../res/labdesk.service tmpdeb/usr/share/labdesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/labdesk.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/labdesk.svg')
    system2(
        'cp ../res/labdesk.desktop tmpdeb/usr/share/applications/labdesk.desktop')
    system2(
        'cp ../res/labdesk-link.desktop tmpdeb/usr/share/applications/labdesk-link.desktop')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/labdesk/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/labdesk/')
    system2(
        'cp ../res/pam.d/labdesk.debian tmpdeb/etc/pam.d/labdesk')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/labdesk/files/polkit && chmod a+x tmpdeb/usr/share/labdesk/files/polkit")
    ...
    system2('dpkg-deb -b tmpdeb labdesk.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('labdesk.deb', '../labdesk-%s.deb' % version)
    if ships_so:
...
DRMTAP_DLOPEN_MARKER = b'/usr/lib/labdesk/libdrmtap.so.0'
...
def _carries_drmtap_marker(path, marker=DRMTAP_DLOPEN_MARKER):
    ...
    # liblabdesk.so is ~45 MB and there is no reason to hold it all in memory, and the `with`
...
def assert_staged_binary_is_drm():
    ...
    """
    binaries = [p for p in glob.glob('tmpdeb/usr/share/labdesk/lib/liblabdesk.so')
                + glob.glob('tmpdeb/usr/share/labdesk/labdesk') if os.path.isfile(p)]
...
def build_deb_from_folder(version, binary_folder, want_drm=False):
    ...
    system2('rm tmpdeb/usr/bin/labdesk || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/labdesk/')
    system2(
        'cp ../res/labdesk.service tmpdeb/usr/share/labdesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/labdesk.png')
    ...
    bundled_glob = glob.glob('tmpdeb/usr/share/labdesk/libdrmtap.so.0.*')
    ...
    if want_drm:
        ...
        if bundle_carries_so:
            ...
            system2(f'rm -f "{so}"')
            system2('rm -f tmpdeb/usr/share/labdesk/libdrmtap.so tmpdeb/usr/share/labdesk/libdrmtap.so.0')
        else:
...
def build_flutter_dmg(version, features):
    ...
    # copy dylib
    system2(
        "cp target/release/libliblabdesk.dylib target/release/liblabdesk.dylib")
    os.chdir('flutter')
    ...
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/LabDesk.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"LabDesk Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon LabDesk.app 200 190 --hide-extension LabDesk.app labdesk.dmg ./build/macos/Build/Products/Release/LabDesk.app")
    os.rename("labdesk.dmg", f"../labdesk-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_arch_manjaro(version, features):
    ...
    os.chdir('flutter')
    system2('flutter build linux --release')
    system2(f'strip {flutter_build_dir}/lib/liblabdesk.so')
    os.chdir('../res')
...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        system2(f'cargo build --locked --features {features} --lib --release')
        if not os.path.exists("target/release/liblabdesk.dll"):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/labdesk.exe')
    os.chdir('../..')
    if os.path.exists('./labdesk_portable.exe'):
        os.replace('./target/release/labdesk-portable-packer.exe',
                   './labdesk_portable.exe')
    else:
        os.rename('./target/release/labdesk-portable-packer.exe',
                  './labdesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/labdesk_portable.exe')
    os.rename('./labdesk_portable.exe', f'./labdesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/labdesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/labdesk-{version}-win7-install.exe')
        system2(f'mv ../../{res_dir}/labdesk-{version}-win7-install.exe ../..')
        ...
        if flutter:
            ...
            system2('cargo build --locked --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/labdesk')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv labdesk-%s-0-x86_64.pkg.tar.zst labdesk-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        # pacman -U ./labdesk.pkg.tar.zst
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/labdesk-%s-0.x86_64.rpm ./labdesk-%s-fedora28-centos8.rpm' % (
                version, version))
        # yum localinstall labdesk.rpm
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/labdesk-%s-0.x86_64.rpm ./labdesk-%s-suse.rpm' % (
                version, version))
        # yum localinstall labdesk.rpm
    else:
        if flutter:
            if osx:
                ...
                # system2(
                #     'mv target/release/bundle/deb/labdesk*.deb ./flutter/labdesk.deb')
                build_flutter_deb(version, features)
        else:
            system2('cargo --locked bundle --release --features ' + features)
            if osx:
                system2(
                    'strip target/release/bundle/osx/LabDesk.app/Contents/MacOS/labdesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/LabDesk.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/labdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/LabDesk.app/Contents/MacOS/labdesk
    #rcodesign sign --p12-file ~/.p12/labdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/LabDesk.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/labdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/LabDesk.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/LabDesk.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/LabDesk.app
    '''.format(pa))
                system2(
                    'create-dmg "LabDesk %s.dmg" "target/release/bundle/osx/LabDesk.app"' % version)
                os.rename('LabDesk %s.dmg' %
                          version, 'labdesk-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/labdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./labdesk-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./labdesk-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple labdesk-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/LabDesk.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                # build deb package
                system2(
                    'mv target/release/bundle/deb/labdesk*.deb ./labdesk.deb')
                system2('dpkg-deb -R labdesk.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/labdesk/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                system2(
                    'cp res/labdesk-link.desktop tmpdeb/usr/share/applications/labdesk-link.desktop')
                os.system('mkdir -p tmpdeb/etc/labdesk/')
                os.system('cp -a res/startwm.sh tmpdeb/etc/labdesk/')
                os.system('mkdir -p tmpdeb/etc/X11/labdesk/')
                os.system('cp res/xorg.conf tmpdeb/etc/X11/labdesk/')
                os.system('cp -a DEBIAN/* tmpdeb/DEBIAN/')
                os.system('mkdir -p tmpdeb/etc/pam.d/')
                os.system('cp pam.d/labdesk.debian tmpdeb/etc/pam.d/labdesk')
                system2('strip tmpdeb/usr/bin/labdesk')
                system2('mkdir -p tmpdeb/usr/share/labdesk')
                system2('mv tmpdeb/usr/bin/labdesk tmpdeb/usr/share/labdesk/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/labdesk/')
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb labdesk.deb; /bin/rm -rf tmpdeb/')
                os.rename('labdesk.deb', 'labdesk-%s.deb' % version)
