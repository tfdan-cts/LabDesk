...
typedef char** (*FUNC_LABDESK_CORE_MAIN)(int*);
typedef void (*FUNC_LABDESK_FREE_ARGS)( char**, int);
typedef int (*FUNC_LABDESK_GET_APP_NAME)(wchar_t*, int);
typedef int (*FUNC_LABDESK_IS_DISABLE_INSTALLATION)();
...
{
  HINSTANCE hInstance = LoadLibraryA("liblabdesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load liblabdesk.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_LABDESK_CORE_MAIN rustdesk_core_main =
      (FUNC_LABDESK_CORE_MAIN)GetProcAddress(hInstance, "rustdesk_core_main_args");
  if (!rustdesk_core_main)
  {
    std::cout << "Failed to get rustdesk_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_LABDESK_FREE_ARGS free_c_args =
      (FUNC_LABDESK_FREE_ARGS)GetProcAddress(hInstance, "free_c_args");
  if (!free_c_args)
  {
  ...
  if (!c_args)
  {
    ...
    // std::cout << "LabDesk [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  std::vector<std::string> rust_args(c_args, c_args + args_len);
  free_c_args(c_args, args_len);
  FUNC_LABDESK_IS_DISABLE_INSTALLATION labdesk_is_disable_installation =
      (FUNC_LABDESK_IS_DISABLE_INSTALLATION)GetProcAddress(hInstance, "rustdesk_is_disable_installation");
  bool is_disable_installation =
      labdesk_is_disable_installation && labdesk_is_disable_installation() != 0;
  ...
  std::wstring app_name = L"LabDesk";
  FUNC_LABDESK_GET_APP_NAME get_labdesk_app_name = (FUNC_LABDESK_GET_APP_NAME)GetProcAddress(hInstance, "get_rustdesk_app_name");
  if (get_labdesk_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_labdesk_app_name(app_name_buffer, 512) == 0) {
